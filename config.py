    ...
    Screen(
        top=bar.Bar(
            [
                ...
                widget.Spacer(background=Colors.BLUE.value, **powerline),
                widget.CurrentLayoutIcon(scale=0.4, padding=10, background=Colors.LIGHTBLUE.value, **powerline),
                widget.Clock(format="%a, %B %d, %Y %H:%M", padding=5, background=Colors.YELLOW.value, foreground=Colors.BLACK.value),
                widget.TextBox(Icons.CLOCK.value, fontsize=20, padding=20, background=Colors.YELLOW.value, foreground=Colors.BLACK.value),
            ],
            32,
        ),
    ),
